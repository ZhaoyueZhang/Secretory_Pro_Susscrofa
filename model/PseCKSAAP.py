...
if __name__ == '__main__':
    ...
    psecksaap.encodings.to_csv('temp.csv')

    #output = 'PseCKSAAP.csv'
    f = open('temp.csv')
    g = open(output,'w')
    count = 1
    for eachline in f:
        if count==1:
            g.write('Class%s'%eachline)
            count += 1
        else:
...
# python3 PseCKSAAP.py --file test.fasta --out test.csv